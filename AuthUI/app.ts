...
import { AuthServer } from "./shared/logic/AuthServer.js";
...
const _server = new AuthServer(document, _dateHelper);
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    _pageWithParams = `${Pages.Login}|`; // page number | 
    await getHtml();
...
document.addEventListener("click", async function (event: Event) {
    ...
    if ((element.id as string).startsWith("_")) { return; }
